...
func Encode(m *Properties, buf *bytes.Buffer) (err error) {
	...
	for i := 0; i < count; i++ {
		...
		case reflect.Uint8:
			...
			err = util.SetUint8(Properties_value[f.Name], bt)
			if err != nil {
				return
			}
			err = util.SetUint8(uint8(v.Uint()), bt)
			if err != nil {
				return
			}
		case reflect.Uint16:
			...
			err = util.SetUint16(uint16(v.Uint()), bt)
			if err != nil {
				return
			}
		case reflect.Uint32:
			...
			err = util.SetUint32(uint32(v.Uint()), bt)
			if err != nil {
				return
			}
		case reflect.String:
			...
			err = util.SetString(v.String(), bt)
			if err != nil {
				return
			}
		case reflect.Slice:
			...
			for _, val := range v.MapKeys() {
				if reflect.TypeOf(v.MapIndex(val).Interface()).Kind() == reflect.Slice {
					...
					for j := 0; j < in.Len(); j++ {
						...
						err = util.SetString(val.String(), bt)
						if err != nil {
							return
						}

						err = util.SetString(util.InterfaceToString(in.Index(j).Interface()), bt)
						if err != nil {
							return
						}
					}
					...
					err = util.SetString(util.InterfaceToString(v.MapIndex(val).Interface()), bt)
					if err != nil {
						return
					}
				}
...
func Decode(m *Properties, b []byte, p *int) {
	...
	l := int(util.DecodeLength(b, p))
	lt := l + *p
	for *p < lt {
		k := Properties_name[util.GetUint8(b, p)]
		field := t.FieldByName(k)
		// fmt.Println(k, b[*p:])
		switch Properties_type[k] {
		case reflect.Uint8:
			field.Set(reflect.ValueOf(util.GetUint8(b, p)))
		case reflect.Uint16:
			field.Set(reflect.ValueOf(util.GetUint16(b, p)))
		case reflect.Uint32:
			field.Set(reflect.ValueOf(util.GetUint32(b, p)))
		case reflect.String:
			field.SetString(util.GetString(b, p))
		case reflect.Slice:
			field.SetBytes(util.GetBytes(b, p))
		case reflect.Map:
			key := util.GetString(b, p)
			val := util.GetString(b, p)
			if field.IsNil() {
